...
export const generateFolderIcons = (color: string | undefined) => {
  ...
  writeSVGFiles('folder', getSVG(getPath(folderIcon, color), 32));
  writeSVGFiles('folder-open', getSVG(getPath(folderIconOpen, color), 32));
  writeSVGFiles('folder-root', getSVG(getPath(rootFolderIcon, color), 32));
  writeSVGFiles(
    'folder-root-open',
    getSVG(getPath(rootFolderIconOpen, color), 32)
  );
};

export const getPath = (d: string, color: string) =>
  `<path d="${d}" fill="${color}" />`;
export const getSVG = (path: string, viewBoxSize: number = 24) =>
